...
def find_las_files_with_back_in_rtc(root_dir):
    las_files = []
    for dirpath, _, filenames in os.walk(root_dir):
        # Only process folders named exactly 'rtc'
        if os.path.basename(dirpath) == "rtc":
            for filename in filenames:
                if filename.lower().endswith('.las') and "back" in filename.lower():
...
if __name__ == "__main__":
    ...
    las_files = find_las_files_with_back_in_rtc(root_dir)
    results = []
